...
class ImageSplitter:
    ...
    def __init__(self, seg_size=48, scale_factor=2, boarder_pad_size=3, device='cpu', img_pad=0):
        self.seg_size = seg_size
        self.scale_factor = scale_factor
        self.pad_size = boarder_pad_size
        self.height = 0
        self.width = 0
        self.upsampler = torch.nn.Upsample(scale_factor=scale_factor, mode='bilinear')

        self.device = device
        if self.device == 'cpu':
            self.zero_pad = torch.nn.ZeroPad2d(img_pad)
            self.replication_pad_2d_a = torch.nn.ReplicationPad2d(self.pad_size)
            self.replication_pad_2d_b = torch.nn.ReplicationPad2d(self.pad_size * self.scale_factor)
        elif self.device == 'cuda:0':
            self.zero_pad = torch.nn.ZeroPad2d(img_pad).cuda(0)
            self.replication_pad_2d_a = torch.nn.ReplicationPad2d(self.pad_size).cuda(0)
            self.replication_pad_2d_b = torch.nn.ReplicationPad2d(self.pad_size * self.scale_factor).cuda(0)
        else:
            raise Exception('This project does not support GPUs other than cuda:0')


    def split_img_tensor(self, pil_img, scale_method=Image.BILINEAR, img_pad=0):
        if self.device == 'cpu':
            # resize image and convert them into tensor
            img_tensor = to_tensor(pil_img).unsqueeze(0).to(self.device)
        else:
            img_tensor = to_tensor(pil_img).unsqueeze(0)

        img_tensor = self.replication_pad_2d_a(img_tensor)
        ...
        if scale_method is not None:
            ...
            img_up = self.replication_pad_2d_b(img_up)
        ...
        for i in range(self.pad_size, height, self.seg_size):
            for j in range(self.pad_size, width, self.seg_size):
                ...
                if img_pad > 0:
                    part = torch.nn.ZeroPad2d(img_pad)(part)
                if scale_method is not None:
    ...
    def merge_img_tensor(self, list_img_tensor):
        if self.device == 'cpu':
            ...
            out = torch.zeros((1, 3, self.height * self.scale_factor, self.width * self.scale_factor)).to(self.device)
        img_tensors = list_img_tensor.copy()
        rem = self.pad_size * 2
...
def load_single_image(img_file,
    ...
    if zero_padding:
        out = torch.nn.ZeroPad2d(zero_padding)(out)
    if up_scale:
