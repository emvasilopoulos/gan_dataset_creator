...
def load_ckpt2(cur_model, ckpt_base_dir, model_name='model', device='cpu', strict=True):
    if os.path.isfile(ckpt_base_dir):
        base_dir = os.path.dirname(ckpt_base_dir)
        ckpt_path = ckpt_base_dir
        checkpoint = torch.load(ckpt_base_dir, map_location=device)
    else:
        base_dir = ckpt_base_dir
        checkpoint, ckpt_path = get_last_checkpoint(ckpt_base_dir)
    if checkpoint is not None:
        ...
        e_msg = f"| ckpt not found in {base_dir}."
        raise Exception("ABORT MISSION: " + e_msg)
